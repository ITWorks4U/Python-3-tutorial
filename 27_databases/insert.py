...
try:
	with conn.connect(host=host, user=name, password=passwd, database=db) as mysql:
		...
		#	insert command
		#	values contains dummy values...
		cmd = 'insert into TestTable (id, description) values (%s, %s)'

		#	...which can be used here
		values = (3, 'third input')
		cursor.execute(cmd, values)

		#	with a commit a change to the certain table can be done,
		#	if everything is well
		mysql.commit()

		#	optionally, you can check, how many new added rows has been inserted
